...
export class CodebaseAnalyzer {
   ...
   */
  async analyzeProject(applicationId: string, projectPath: string): Promise<CodebaseAnalysis> {
  ...
  private detectProjectType(fileStructure: FileStructureAnalysis): {
    type: string;
    languages: string[];
    frameworks: string[];
    architecture: string;
    ...
    // Language detection
    if (fileTypes['.ts'] || fileTypes['.tsx']) languages.push('typescript');
    if (fileTypes['.js'] || fileTypes['.jsx']) languages.push('javascript');
    if (fileTypes['.py']) languages.push('python');
    if (fileTypes['.go']) languages.push('go');
    if (fileTypes['.rs']) languages.push('rust');
    if (fileTypes['.java']) languages.push('java');
    if (fileTypes['.php']) languages.push('php');

    // Framework detection based on config files
    const configFileNames = configFiles.map(f => path.basename(f));
    
    if (configFileNames.includes('package.json')) {
      if (configFileNames.includes('next.config.js')) frameworks.push('nextjs');
      if (configFileNames.includes('nuxt.config.js')) frameworks.push('nuxt');
      if (fileTypes['.vue']) frameworks.push('vue');
      if (fileTypes['.svelte']) frameworks.push('svelte');
      if (fileTypes['.tsx'] || fileTypes['.jsx']) frameworks.push('react');
    }
    
    if (configFileNames.includes('requirements.txt') || configFileNames.includes('pyproject.toml')) {
      if (configFileNames.some(f => f.includes('fastapi'))) frameworks.push('fastapi');
      if (configFileNames.some(f => f.includes('django'))) frameworks.push('django');
      if (configFileNames.some(f => f.includes('flask'))) frameworks.push('flask');
    }
  ...
  private detectPatterns(fileStructure: FileStructureAnalysis): DetectedPattern[] {
  ...
  private categorizeAgents(suggestions: AgentSuggestion[]): AgentSuggestion[] {
  ...
  private assessComplexity(
    ...
    // File count scoring
    if (fileStructure.totalFiles > 1000) score += 3;
    else if (fileStructure.totalFiles > 500) score += 2;
    else if (fileStructure.totalFiles > 100) score += 1;
    
    // Directory depth scoring
    if (fileStructure.directoryDepth > 8) score += 2;
    else if (fileStructure.directoryDepth > 5) score += 1;
  ...
  private async storeAnalysis(analysis: CodebaseAnalysis): Promise<void> {
    await this.db.collection('codebase_analyses').replaceOne(
